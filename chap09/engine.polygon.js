var Polygon = function() {
	...
	this.sList = null;
	this.sCount = null;
	// intensity color of each vertex
	this.intensity = null;
	// store here the z clipeed calculated intensities
...
Polygon.prototype = {
	...
	doClipZNear: function() {
		...
		// store here the new intensities values
		var newIntensity = [];
		// a reference to intensities
		...
		for( ; endIndex < 4; endIndex++ ) {
			...
			if( startVertex.wz >= Engine.MINZ ) {
				if( endVertex.wz >= Engine.MINZ ) {
					...
					newIntensity[vcCount] = intensity[endIndex];
					// update index
					vcCount++;
					...
					newIntensity[vcCount] = intensity[startIndex] + ( intensity[endIndex] - intensity[startIndex] ) * t;
					// update index
					vcCount++;
				}
	...
	project: function() {
		...
		for( count = 0; count < this.vcCount; count++ ) {
			...
			sVertex.z = oneOverZ;
			sVertex.intensity = this.newIntensity[count];
		...
		while( edgeCount > 0 ) {
			...
			while( height-- > 0 ) {
				...
				if( width > 0 ) {
					...
					// clip the scan-line
					var res = LineSegment.clipHLine( xStart, xEnd, z, zStep, i, iStep );
					xStart = res[0];
					xEnd = res[1];
					z = res[2];
					i = res[3];
					...
					while( width-- > 0 ) {
						if( zBuffer[zPointer] < z ) {
							zBuffer[zPointer] = z;
							screen[screenPointer] = Math.round( i );
						}
						z += zStep;
						i += iStep;
						++screenPointer;
