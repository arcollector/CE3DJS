...
Polygon.prototype = {
	...
	project: function() {
		...
		for( count = 0; count < this.vcCount; count++ ) {
			...
			sVertex.z = oneOverZ;
			sVertex.u = this.newA[count].u;
			sVertex.v = this.newA[count].v;
	...
	displayNormal: function( videoMem, zBuffer ) {
		...
		var textureWidth = Engine.Textures.tMap[this.bindedTexture].width;
		//var textureWidth = Math.log( Engine.Textures.tMap[this.bindedTexture].width ) / Math.log( 2 );
		...
		while( edgeCount > 0 ) {
			...
			while( height-- > 0 ) {
				...
				if( width > 0 ) {
					...
					// clip the scan-line
					var res = LineSegment.clipHLine( xStart, xEnd, z, zStep, u, uStep, v, vStep );
					xStart = res[0];
					xEnd = res[1];
					z = res[2];
					u = res[3];
					...
					while( width-- > 0 ) {
						if( zBuffer[zPointer] < z ) {
							zBuffer[zPointer] = z;
							var iPos = Math.round( v ) * textureWidth + Math.round( u );
							videoMem[videoPointer] = iPtr[iPos];
						}
						z += zStep;
						u += uStep;
						v += vStep;
				...
				if( width > 0 ) {
					...
					while( width-- > 0 ) {
						if( zBuffer[zPointer] < z ) {
							zBuffer[zPointer] = z;
							var iPos = Math.round( v ) * textureWidth + Math.round( u );
							var colorIndex = iPtr[iPos] + 1;
							if( colorIndex > Engine.COLOR_RANGE ) {
								colorIndex -= Engine.COLOR_RANGE;
							}
