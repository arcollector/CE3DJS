...
var draw = function( timestamp ) {
	...
	monster.moveToward( 0, -150, 0, 100, m );
	...
	if( (monsterFrameCount % 4) === 0 ) {
		...
		// the "walking" sequence
			...
			// if the monster is coloser thatn a distance of 500
			if( monster.dist() < 500 ) {
				if( !noSoundCount ) {
					// if it is, we calculate a random number that lies somewhere in
					// in the range of 0 to 100, and play one of two WAVE sounds
					// based on its result
					if( Math.floor(Math.random()*(100-0+1)+0) >= 50 ) {
						Engine.Sound.play( 'pain1' );
					} else {
						Engine.Sound.play( 'pain2' );
					}
					// then we set noSoundCount to 20, which prevents us from
					// playing a sound for the next 20 frames (the variable noSoundCount,
					// if it's greater than 0, is decremented each cycle through the
					// skeleton's frames)
					noSoundCount = 20;
				}
			}
...
var main = function() {
	...
	Engine.Sound.load( 'pain1', 'PAIN1.WAV' );
	Engine.Sound.load( 'pain2', 'PAIN2.WAV' );
